;; don't edit start
(#t #t)
(mosh-only ("<transcoded-textual-input-port <binary-input-port src/all-tests.scm>>" 2) (source-info '(3)))
;; don't edit end
;; test start
(#t (and))
(3 3)
...
[3
 (receive (a b)
     (with-input-from-file "src/all-tests.scm" (lambda () (values 1 2)))
   (+ a b))]
[7
 (let ([x 3])
   (define a 4)
...
(#f    (port-transcoder (open-file-input-port "./src/test-data.scm" (make-file-options '()) 'block #f)))
...
;; R6RS round returns inexact value for inexact input. 
(0.0 (round 0.4))
(0.0 (round 0.5))
(1.0 (round 0.6))
(1.0 (round 1.0))
