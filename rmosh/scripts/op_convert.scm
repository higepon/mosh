  ...
  (case-lambda
    ...
    [(insn* idx)
      (match insn*
        ...
        [((and (or 'FRAME 'TEST 'LOCAL_JMP 'BRANCH_NOT_GE 'BRANCH_NOT_NUMBER_EQUAL) insn) offset . more*)
        ...
        [((and (or 'CALL 'DISPLAY 'LEAVE 'LET_FRAME 'RETURN 'ASSIGN_FREE 'REFER_FREE 'REFER_LOCAL 'ASSIGN_LOCAL 'FRAME 'REFER_LOCAL) insn) n . more*)
          (format #t "            Op::~a(~a),\n" (insn->string insn) n)
          (rewrite-insn* more* (+ idx 2))]
        [((and (or 'HALT 'NOT 'CAR 'CDR 'CADR 'CONS 'NUMBER_EQUAL 'NUMBER_GT 'BRANCH_NOT_GE 'NOP 'NULL_P 'INDIRECT 'PUSH 'NUMBER_ADD 'UNDEF) insn) . more*)
          (format #t "            Op::~a,\n" (insn->string insn))
