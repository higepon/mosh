...
;;;; .title Reference Manual of the bigloo LAML compatibility libray.
;;;; bigloo specific stuff to be loaded for compatibility. 
...
;;;; in LAML already happens to exist in bigloo. These are 
;;;; file-exists?, delete-file, and copy-file.
...
(define directory-list directory->list)

; ---------------------------------------------------------------------------------------------------

;; Mail sending support: Send an email to a receiver.
;; Not implemented on bigloo on the PC platform.
(define (mail receiver title contents)
  (error "The mail function is not implemented in bigloo on the PC platform"))
...
(define (laml-canonical-command-line)
  ...
  (let ((argv (list-tail (command-line) 6)))
    (if (>= (length argv) 2)
      (list 'laml 
            (file-name-proper (list-ref argv 0)) 
            (list-ref argv 1) ; omitted: ensure forward slashing
            (list-tail argv 2))
      #f)))
